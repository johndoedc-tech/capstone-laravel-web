<x-app-layout>
    ...
    <div class="farmer-pwa py-3 lg:py-6 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            <!-- Filters -->
            <div class="farmer-card bg-white overflow-hidden shadow-sm rounded-2xl mb-4 lg:mb-6">
                <div class="p-4 lg:p-6">
                    <p class="farmer-section-kicker mb-1">Saved Estimates</p>
                    <h1 class="mb-1 text-xl font-bold text-gray-900">Your forecast history</h1>
                    <p class="mb-4 text-sm text-gray-500">Filter old estimates or start a new one.</p>
                    <form method="GET" action="{{ route('predictions.history') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 lg:gap-4">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm lg:text-base">
                                ...
                                <option value="failed" {{ request('status') == 'failed' ? 'selected' : '' }}>Needs Retry</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="prediction_type" name="prediction_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm lg:text-base">
                                ...
                                <option value="regular" {{ request('prediction_type') == 'regular' ? 'selected' : '' }}>Harvest Estimate</option>
                ...
                <div class="p-4 lg:p-6">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 text-sm lg:text-base">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Change</th>
                                        <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Model Score</th>
                    ...
                    @else
                        <div class="text-center py-8 lg:py-12">
                            ...
                            <h3 class="mt-2 text-sm lg:text-base font-medium text-gray-900">No saved estimates yet</h3>
                            <p class="mt-1 text-xs lg:text-sm text-gray-500">Check one crop first.</p>
                            <div class="mt-4 lg:mt-6">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mt-4">
                        <div id="chartLoading" class="flex items-center justify-center py-12">
                            ...
                            <span class="ml-2 text-gray-600">Loading forecast...</span>
                        </div>
                        ...
                        <div id="chartError" class="hidden text-center py-8 text-red-600">
                            ...
                            <p class="mt-2" id="chartErrorMessage">Could not load forecast.</p>
                        </div>
                    </div>
    ...
    <script>
        ...
        function viewForecastChart(batchId) {
            ...
            document.getElementById('modal-subtitle').textContent = 'Loading forecast...';
            ...
            fetch(`{{ route('predictions.forecast-batch') }}?batch_id=${encodeURIComponent(batchId)}`)
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('chartErrorMessage').textContent = data.error || 'Could not load forecast.';
                    }
                })
                .catch(error => {
                    document.getElementById('chartLoading').classList.add('hidden');
                    document.getElementById('chartError').classList.remove('hidden');
                    document.getElementById('chartErrorMessage').textContent = 'Connection problem: ' + error.message;
                });
        }
        ...
        function clearHistory() {
            ...
            .then(data => {
                if (data.success) {
                    window.harvianaToast?.('History cleared', 'Your prediction history has been cleared.');
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                Clear Saved Forecast History
                            </h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Clear saved estimates and forecasts? This cannot be undone.
                                </p>
                            </div>
                        </div>
                    </div>
