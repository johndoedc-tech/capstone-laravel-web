<x-admin-layout>
                ...
                <div class="p-4 lg:p-6">
                    <form method="GET" action="{{ route('admin.predictions.history') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 lg:gap-4">
                        ...
                        <div class="sm:col-span-2 lg:col-span-6 flex flex-col sm:flex-row gap-2">
                            ...
                            <a href="{{ route('admin.predictions.index') }}" class="inline-flex items-center justify-center px-4 py-2 bg-primary border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 active:bg-primary-800 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        ...
        function viewForecastChart(batchId) {
            ...
            // Fetch forecast data
            fetch(`{{ route('admin.predictions.forecast-batch') }}?batch_id=${encodeURIComponent(batchId)}`)
        ...
        function clearHistory() {
            ...
            fetch('{{ route('admin.predictions.clear-history') }}', {
                method: 'DELETE',
                headers: {
