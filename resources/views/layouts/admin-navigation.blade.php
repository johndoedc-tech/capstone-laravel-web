...
@include('layouts.mobile-header', ['homeRoute' => route('admin.dashboard'), 'sectionLabel' => 'Admin Panel'])
...
<aside x-cloak class="mobile-sidebar w-64 flex flex-col fixed left-0 top-0 z-40 transform transition-transform duration-300 ease-in-out lg:translate-x-0" style="background-color: #355872;" :class="{ '-translate-x-full': !open, 'translate-x-0': open }">
    ...
    <nav class="sidebar-scroll flex-1 p-3 lg:p-4 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('admin.lgu-validators.index') }}" class="flex items-center gap-3 px-3 lg:px-4 py-2.5 lg:py-3 rounded-lg text-sm lg:text-base {{ request()->routeIs('admin.lgu-validators.*') ? 'bg-primary text-white' : 'text-cream hover:bg-primary-700/50' }}">
            <svg class="w-4 h-4 lg:w-5 lg:h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Mobile overlay -->
<div x-show="open" x-cloak @click="open = false; window.dispatchEvent(new CustomEvent('harviana-sidebar-theme', { detail: { open: false } }))" class="mobile-overlay-safe lg:hidden fixed bg-black bg-opacity-50 z-30"></div>
</div>
