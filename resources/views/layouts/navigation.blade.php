...
<aside class="w-64 flex flex-col h-screen fixed left-0 top-0 z-40 transform transition-transform duration-300 ease-in-out lg:translate-x-0" style="background-color: #355872;" :class="{ '-translate-x-full': !open, 'translate-x-0': open }">
    ...
    <div class="p-4 lg:p-6 border-b border-primary-700/30">
        <div class="flex items-start gap-3">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center justify-between gap-3 flex-1 min-w-0">
                <div class="min-w-0">
                    <h1 class="font-bold text-white text-sm lg:text-lg leading-tight truncate">Harviana</h1>
                    <p class="text-[11px] lg:text-xs text-primary-200 leading-tight">Agricultural System</p>
                </div>
...
<button x-show="!open" @click="open = true" class="lg:hidden fixed top-3 left-3 z-50 p-2.5 rounded-xl shadow-lg border border-primary-700/30 hover:bg-primary-600 active:scale-95 transition-transform" style="background-color: #355872;">
